...
@RequestMapping("ch05/ex01") // 과거의 Controller는 view가 중요했지만, RESTful에서는 데이터만 전달하므로 중요하지않다.
public class UserController {
	...
	@GetMapping("get") //@GET은 데이터가 안넘겨질수있다. view만 넘길수있어, request에 대해 데이터를 넘겨주는걸 parameter라고 한다. 
	public User get(User user) { // 이 커맨더는 @requestParam이 생략된것.
		return user; // RESTful에서 model을 return한다? json으로 변환해서 자동으로 return해줌(key : value) 
	}
	
	//POST방식은 request에 담는 parameter가 body에 담긴다.
	@PostMapping("post") 
	public User post(@RequestBody User user) { //@requestParam, @CookieValue, @PathVariable, @RequestBody
		return user;
	}

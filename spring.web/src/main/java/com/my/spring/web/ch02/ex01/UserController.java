...
@Controller //@Component를 포함하므로 Ioc로 자동등록됨.
@RequestMapping("ch02/ex01") // request를 받기위한 url
public class UserController {
	@GetMapping("11") // 어떤메서드가 request를 받을지 설정하는 url
	public ModelAndView handler11(ModelAndView mv) { // handler(request를 handling 하는 메서드)
		mv.addObject("user", new User("최한석", 11)); // modelName: user, modelValue: new User
		mv.setViewName("ch02/ex01/user"); // viewName, tomcat이 prefix,suffi 붙여서 url을 mapping해서 찾아감. 

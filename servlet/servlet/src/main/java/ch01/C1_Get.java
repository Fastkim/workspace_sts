...
public class C1_Get extends HttpServlet { // HttpServlet: Http에 관련된 알고리즘을 구현한 메서드를 가지고 있는 클래스이다.
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		//url을 알려주고 encoding을 알려준다. http text는 physical 하게 text이지만 logical 하게는 content이다.
		...
		// tomcat server, oracle server가 돌아가고있다. request는 동일한 컴퓨터(노드)에 도달하고 포트번호로 두개의 서버중 어디로 보낼지 정하는것이다.
		...
		// or-maping: physical한 데이터의 차이점을 매꾸려고 사용 (physical한 데이터 -> oracle은 레코드, java는 객체)
		// tomcat http text타입의 request를 request객체로 만들어서 처리, 즉 tomcat도 or-maping을 수행하고있는것이다.
		// 그럼 object text-mapping(ot mapping)아님, 아니면 object http - mapping(oh Mapping)이거나
		// http servlet type 객체
		// C1_Get 클래스(객체)는 httpServelet, Object, 
